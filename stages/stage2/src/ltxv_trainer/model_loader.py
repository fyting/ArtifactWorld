...
class LtxvModelVersion(str, Enum):
    ...
    LTXV_13B_098_DISTILL = "LTXV_13B_098_DISTILL"
    LTXV_2B_098_DISTILL = "LTXV_2B_098_DISTILL"
    ...
    def hf_repo(self) -> str:
        ...
        match self:
            ...
            case LtxvModelVersion.LTXV_13B_098_DISSTILL:
                return "Lightricks/LTX-Video-0.9.8-disstill"
...
def load_scheduler(source: str | Path | None = None) -> FlowMatchEulerDiscreteScheduler:
    """
    ...
    Returns:
        Loaded scheduler
    """
    source = source or LtxvModelVersion.LTXV_13B_097_DEV.hf_repo
...
def load_tokenizer(source: str | Path | None = None) -> T5Tokenizer:
    """
...
def load_text_encoder(source: str | Path | None = None, *, load_in_8bit: bool = False) -> T5EncoderModel:
    """
...
def load_vae(
    ...
    *,
    ...
    if isinstance(source, LtxvModelVersion):
        ...
        elif source in (LtxvModelVersion.LTXV_13B_098_DISTILL,
                        LtxvModelVersion.LTXV_2B_098_DISTILL):
            return AutoencoderKLLTXVideo.from_pretrained(
                "LTX-Video-vae",
                subfolder="vae",
                torch_dtype=dtype,
    ...
    *,
    ...
    if isinstance(source, LtxvModelVersion):
        ...
        if source in (
            LtxvModelVersion.LTXV_13B_097_DEV,
            LtxvModelVersion.LTXV_13B_097_DISTILLED,
            LtxvModelVersion.LTXV_13B_098_DISTILL,
        ):
...
def load_ltxv_components(
    ...
    *,
    scheduler_source: str | Path | None = None,
    tokenizer_source: str | Path | None = None,
    text_encoder_source: str | Path | None = None,
    vae_source: ModelSource | None = None,
    transformer_source: ModelSource | None = None,
    ...
    return LtxvModelComponents(
        scheduler=load_scheduler(scheduler_source),
        tokenizer=load_tokenizer(tokenizer_source),
        text_encoder=load_text_encoder(text_encoder_source, load_in_8bit=load_text_encoder_in_8bit),
        vae=load_vae(vae_source or model_source, dtype=vae_dtype),
        transformer=load_transformer(transformer_source or model_source, dtype=transformer_dtype),
    )
