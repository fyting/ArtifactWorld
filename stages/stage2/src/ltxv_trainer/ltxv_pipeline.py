...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        noisemap_video: Optional[torch.Tensor] = None,
        generate_noisemap: bool = False,
        auxiliary_latent_path: Optional[str] = None,
        auxiliary_latent_b_path: Optional[str] = None,
        noisemap_blend_weights: Optional[List[tuple[float, float, float]]] = None,
        save_noisemap_latents: bool = False,
        noisemap_latents_save_path: Optional[str] = None,
        ...
        if reference_video is not None:
            ...
            # ✅ MODIFIED: If generate_noisemap is enabled, use provided noisemap_video as conditioning
            noisemap_latents = None
            auxiliary_latents = None
            noisemap_num_latents = 0
            if generate_noisemap:
                if noisemap_video is None:
                    raise ValueError("generate_noisemap=True requires noisemap_video to be provided")

                # ✅ NEW: Load auxiliary latent A if provided
                if auxiliary_latent_path is not None:
                    auxiliary_data = torch.load(auxiliary_latent_path, map_location=device)

                    # Handle packed latent format (dict with 'latents' key)
                    if isinstance(auxiliary_data, dict) and 'latents' in auxiliary_data:
                        # This is already packed format: [seq_len, channels]
                        auxiliary_latents_packed = auxiliary_data['latents']
                        # Ensure batch dimension: [B, seq_len, channels]
                        if auxiliary_latents_packed.ndim == 2:
                            auxiliary_latents_packed = auxiliary_latents_packed.unsqueeze(0)
                        # Move to correct device and dtype
                        auxiliary_latents_packed = auxiliary_latents_packed.to(device, dtype=torch.float32)
                        auxiliary_latents = None  # Mark as already packed
                    elif isinstance(auxiliary_data, torch.Tensor):
                        # This is unpacked format [C, F, H, W] or [B, C, F, H, W]
                        auxiliary_latents = auxiliary_data
                        if auxiliary_latents.ndim == 4:
                            auxiliary_latents = auxiliary_latents.unsqueeze(0)
                        auxiliary_latents = auxiliary_latents.to(device, dtype=torch.float32)
                        auxiliary_latents_packed = None  # Will pack later
                    else:
                        raise ValueError(
                            f"Auxiliary latent file must contain either a dict with 'latents' key "
                            f"or a tensor, but got {type(auxiliary_data)}"
                        )

                # ✅ NEW: Load auxiliary latent B if provided
                auxiliary_latents_b = None
                auxiliary_latents_b_packed = None
                if auxiliary_latent_b_path is not None:
                    auxiliary_data_b = torch.load(auxiliary_latent_b_path, map_location=device)

                    # Handle packed latent format (dict with 'latents' key)
                    if isinstance(auxiliary_data_b, dict) and 'latents' in auxiliary_data_b:
                        # This is already packed format: [seq_len, channels]
                        auxiliary_latents_b_packed = auxiliary_data_b['latents']
                        # Ensure batch dimension: [B, seq_len, channels]
                        if auxiliary_latents_b_packed.ndim == 2:
                            auxiliary_latents_b_packed = auxiliary_latents_b_packed.unsqueeze(0)
                        # Move to correct device and dtype
                        auxiliary_latents_b_packed = auxiliary_latents_b_packed.to(device, dtype=torch.float32)
                        auxiliary_latents_b = None  # Mark as already packed
                    elif isinstance(auxiliary_data_b, torch.Tensor):
                        # This is unpacked format [C, F, H, W] or [B, C, F, H, W]
                        auxiliary_latents_b = auxiliary_data_b
                        if auxiliary_latents_b.ndim == 4:
                            auxiliary_latents_b = auxiliary_latents_b.unsqueeze(0)
                        auxiliary_latents_b = auxiliary_latents_b.to(device, dtype=torch.float32)
                        auxiliary_latents_b_packed = None  # Will pack later
                    else:
                        raise ValueError(
                            f"Auxiliary latent B file must contain either a dict with 'latents' key "
                            f"or a tensor, but got {type(auxiliary_data_b)}"
                        )

                # ✅ Process noisemap_video (similar to reference_video processing)
                # Work with the original tensor format [F, C, H, W]
                noisemap_frames = noisemap_video  # [F, C, H, W]

                # Resize maintaining aspect ratio (resize all frames)
                current_height, current_width = noisemap_frames.shape[2:]
                aspect_ratio = current_width / current_height
                target_aspect_ratio = width / height
                ...
                noisemap_frames = resize(noisemap_frames, [resize_height, resize_width], antialias=True)

                # Center crop to target dimensions
                noisemap_frames = center_crop(noisemap_frames, [height, width])

                # Convert to VAE input format: [1, C, F, H, W] and proper range [-1, 1]
                noisemap_tensor = noisemap_frames.unsqueeze(0).permute(0, 2, 1, 3, 4)
                noisemap_tensor = noisemap_tensor * 2.0 - 1.0  # [0, 1] -> [-1, 1]

                # Trim noisemap to proper frame count for temporal compression
                noisemap_num_frames_input = noisemap_tensor.size(2)
                noisemap_num_frames_output = self.trim_conditioning_sequence(0, noisemap_num_frames_input, num_frames)
                noisemap_tensor = noisemap_tensor[:, :, :noisemap_num_frames_output]
                noisemap_tensor = noisemap_tensor.to(device, dtype=vae_dtype)

                # Ensure proper frame count for VAE temporal compression
                if noisemap_tensor.size(2) % self.vae_temporal_compression_ratio != 1:
                    noisemap_frames_to_keep = (
                        (noisemap_tensor.size(2) - 1) // self.vae_temporal_compression_ratio
                    ) * self.vae_temporal_compression_ratio + 1
                    noisemap_tensor = noisemap_tensor[:, :, :noisemap_frames_to_keep]

                # Expand noisemap tensor for batch and num_videos_per_prompt
                noisemap_tensor = noisemap_tensor.repeat(batch_size * num_videos_per_prompt, 1, 1, 1, 1)

                # Encode noisemap to latents
                noisemap_latents = retrieve_latents(self.vae.encode(noisemap_tensor), generator=generator)
                noisemap_latents = self._normalize_latents(
                    noisemap_latents, self.vae.latents_mean, self.vae.latents_std
                ).to(device, dtype=torch.float32)

                # Create coordinates for noisemap (same structure as reference)
                noisemap_latent_frames = noisemap_latents.size(2)
                noisemap_latent_height = noisemap_latents.size(3)
                noisemap_latent_width = noisemap_latents.size(4)

                noisemap_video_coords = self._prepare_video_ids(
                    batch_size * num_videos_per_prompt,
                    noisemap_latent_frames,
                    noisemap_latent_height,
                    noisemap_latent_width,
                    patch_size_t=self.transformer_temporal_patch_size,
                    patch_size=self.transformer_spatial_patch_size,
                    device=device,
                )
                noisemap_video_coords = self._scale_video_ids(
                    noisemap_video_coords,
                    scale_factor=self.vae_spatial_compression_ratio,
                    scale_factor_t=self.vae_temporal_compression_ratio,
                    frame_index=0,
                    device=device,
                ...
                # Pack noisemap latents
                noisemap_latents_unpacked = noisemap_latents  # Keep unpacked version for saving
                noisemap_latents = self._pack_latents(
                    noisemap_latents,
                    self.transformer_spatial_patch_size,
                    self.transformer_temporal_patch_size,
                )
                noisemap_num_latents = noisemap_latents.size(1)

                # ✅ NEW: Save noisemap latents if requested
                if save_noisemap_latents and noisemap_latents_save_path is not None:
                    logger.info(f"Saving noisemap latents to {noisemap_latents_save_path}")
                    save_data = {
                        'latents': noisemap_latents[0].cpu(),  # Save first batch item, move to CPU
                        'latents_unpacked': noisemap_latents_unpacked[0].cpu(),  # Also save unpacked version
                        'num_frames': noisemap_latent_frames,
                        'height': noisemap_latent_height,
                        'width': noisemap_latent_width,
                        'fps': frame_rate,
                        'dtype': str(noisemap_latents.dtype),
                        'shape_packed': list(noisemap_latents[0].shape),
                        'shape_unpacked': list(noisemap_latents_unpacked[0].shape),
                    }
                    torch.save(save_data, noisemap_latents_save_path)
                    logger.info(f"✅ Saved noisemap latents with shape {noisemap_latents[0].shape}")

                # ✅ NEW: Pack auxiliary latents if provided
                if auxiliary_latent_path is not None:
                    if auxiliary_latents_packed is None:
                        # Need to pack unpacked latents
                        # Expand auxiliary tensor for batch and num_videos_per_prompt if needed
                        if auxiliary_latents.size(0) != batch_size * num_videos_per_prompt:
                            auxiliary_latents = auxiliary_latents.repeat(batch_size * num_videos_per_prompt, 1, 1, 1, 1)

                        auxiliary_latents_packed = self._pack_latents(
                            auxiliary_latents,
                            self.transformer_spatial_patch_size,
                            self.transformer_temporal_patch_size,
                        )
                    else:
                        # Already packed, just expand for batch if needed
                        if auxiliary_latents_packed.size(0) != batch_size * num_videos_per_prompt:
                            auxiliary_latents_packed = auxiliary_latents_packed.repeat(
                                batch_size * num_videos_per_prompt, 1, 1
                            )

                    # Verify shapes match for blending
                    if auxiliary_latents_packed.shape != noisemap_latents.shape:
                        raise ValueError(
                            f"Auxiliary latents shape {auxiliary_latents_packed.shape} does not match "
                            f"noisemap latents shape {noisemap_latents.shape}. "
                            f"Make sure the auxiliary latent file matches the noisemap video dimensions."
                        )

                    # ✅ DEBUG: Compare auxiliary latents with noisemap latents (COMMENTED OUT)
                    # logger.info("=" * 80)
                    # logger.info("DEBUG: Comparing noisemap_latents (from video) with auxiliary_latents (from .pt file)")
                    # logger.info(f"noisemap_latents shape: {noisemap_latents.shape}")
                    # logger.info(f"noisemap_latents dtype: {noisemap_latents.dtype}")
                    # logger.info(f"noisemap_latents device: {noisemap_latents.device}")
                    # logger.info(f"noisemap_latents mean: {noisemap_latents.mean().item():.6f}")
                    # logger.info(f"noisemap_latents std: {noisemap_latents.std().item():.6f}")
                    # logger.info(f"noisemap_latents min: {noisemap_latents.min().item():.6f}")
                    # logger.info(f"noisemap_latents max: {noisemap_latents.max().item():.6f}")
                    # logger.info("-" * 80)
                    # logger.info(f"auxiliary_latents_packed shape: {auxiliary_latents_packed.shape}")
                    # logger.info(f"auxiliary_latents_packed dtype: {auxiliary_latents_packed.dtype}")
                    # logger.info(f"auxiliary_latents_packed device: {auxiliary_latents_packed.device}")
                    # logger.info(f"auxiliary_latents_packed mean: {auxiliary_latents_packed.mean().item():.6f}")
                    # logger.info(f"auxiliary_latents_packed std: {auxiliary_latents_packed.std().item():.6f}")
                    # logger.info(f"auxiliary_latents_packed min: {auxiliary_latents_packed.min().item():.6f}")
                    # logger.info(f"auxiliary_latents_packed max: {auxiliary_latents_packed.max().item():.6f}")
                    # logger.info("-" * 80)

                    # # Calculate difference
                    # diff = torch.abs(noisemap_latents - auxiliary_latents_packed)
                    # logger.info(f"Absolute difference mean: {diff.mean().item():.6f}")
                    # logger.info(f"Absolute difference max: {diff.max().item():.6f}")
                    # logger.info(f"Relative difference (%): {(diff.mean() / (torch.abs(noisemap_latents).mean() + 1e-8) * 100).item():.2f}%")

                    # # Check if they are nearly identical
                    # is_close = torch.allclose(noisemap_latents, auxiliary_latents_packed, rtol=1e-3, atol=1e-5)
                    # logger.info(f"Are they close (rtol=1e-3, atol=1e-5)? {is_close}")

                    # if not is_close:
                    #     logger.warning("⚠️  WARNING: noisemap_latents and auxiliary_latents are NOT numerically close!")
                    #     logger.warning("This may indicate they were encoded differently or from different videos.")
                    # else:
                    #     logger.info("✅ noisemap_latents and auxiliary_latents are numerically close")
                    # logger.info("=" * 80)

                    # Store for per-step blending in denoising loop
                    # We'll blend later in the loop, so save both
                    self._auxiliary_latents_packed = auxiliary_latents_packed
                    self._noisemap_latents_base = noisemap_latents.clone()

                    # Set flag for debug output
                    self._using_heatmap_blending = noisemap_blend_weights is not None
                    self._blend_weights = noisemap_blend_weights
                else:
                    self._auxiliary_latents_packed = None
                    self._noisemap_latents_base = None
                    self._using_heatmap_blending = False
                    self._blend_weights = None

                # ✅ NEW: Pack auxiliary latents B if provided
                if auxiliary_latent_b_path is not None:
                    if auxiliary_latents_b_packed is None:
                        # Need to pack unpacked latents
                        # Expand auxiliary tensor for batch and num_videos_per_prompt if needed
                        if auxiliary_latents_b.size(0) != batch_size * num_videos_per_prompt:
                            auxiliary_latents_b = auxiliary_latents_b.repeat(batch_size * num_videos_per_prompt, 1, 1, 1, 1)

                        auxiliary_latents_b_packed = self._pack_latents(
                            auxiliary_latents_b,
                            self.transformer_spatial_patch_size,
                            self.transformer_temporal_patch_size,
                        )
                        ...
                        # Already packed, just expand for batch if needed
                        if auxiliary_latents_b_packed.size(0) != batch_size * num_videos_per_prompt:
                            auxiliary_latents_b_packed = auxiliary_latents_b_packed.repeat(
                                batch_size * num_videos_per_prompt, 1, 1
                            )

                    # Verify shapes match for blending
                    if auxiliary_latents_b_packed.shape != noisemap_latents.shape:
                        raise ValueError(
                            f"Auxiliary latents B shape {auxiliary_latents_b_packed.shape} does not match "
                            f"noisemap latents shape {noisemap_latents.shape}. "
                            f"Make sure the auxiliary latent B file matches the noisemap video dimensions."
                        )

                    # Store for per-step blending in denoising loop
                    self._auxiliary_latents_b_packed = auxiliary_latents_b_packed
                else:
                    self._auxiliary_latents_b_packed = None

                # ✅ Apply feature scaling to match training (critical for train-inference consistency)
                noisemap_scale = getattr(self, 'noisemap_feature_scale', 1.0)
                if noisemap_scale != 1.0:
                    noisemap_latents = noisemap_latents * noisemap_scale
                    if self._noisemap_latents_base is not None:
                        self._noisemap_latents_base = self._noisemap_latents_base * noisemap_scale
                    if self._auxiliary_latents_packed is not None:
                        self._auxiliary_latents_packed = self._auxiliary_latents_packed * noisemap_scale
                    if self._auxiliary_latents_b_packed is not None:
                        self._auxiliary_latents_b_packed = self._auxiliary_latents_b_packed * noisemap_scale

                # ✅ Concatenate noisemap latents: [reference_latents, frame_conditions, target_latents, noisemap_latents]
                # Sequence: [ref, target, noisemap] (same as training)
                latents = torch.cat([latents, noisemap_latents], dim=1)

                # ✅ Update video coordinates to include noisemap
                reference_video_coords = reference_video_coords.float()
                video_coords = torch.cat([reference_video_coords, video_coords, noisemap_video_coords], dim=2)
            ...
            # ✅ MODIFIED: Update conditioning mask to include reference and noisemap (both frozen = strength 1.0)
            if conditioning_mask is not None:
                ...
                # ✅ If generate_noisemap, add ones for noisemap tokens (conditioning, no denoising)
                if generate_noisemap:
                    noisemap_conditioning_mask = torch.ones(
                        (batch_size * num_videos_per_prompt, noisemap_num_latents), device=device, dtype=torch.float32
                    )
                    conditioning_mask = torch.cat([conditioning_mask, noisemap_conditioning_mask], dim=1)
            else:
                ...
                target_len = latents.size(1) - reference_num_latents
                if generate_noisemap:
                    target_len = target_len - noisemap_num_latents
                target_conditioning_mask = torch.zeros(
                    (batch_size * num_videos_per_prompt, target_len),
                    device=device,
                    dtype=torch.float32,
                )
                conditioning_mask = torch.cat([conditioning_mask, target_conditioning_mask], dim=1)

                # ✅ Add ones for noisemap if enabled
                if generate_noisemap:
                    noisemap_conditioning_mask = torch.ones(
                        (batch_size * num_videos_per_prompt, noisemap_num_latents),
                        device=device,
                        dtype=torch.float32,
                    )
                    conditioning_mask = torch.cat([conditioning_mask, noisemap_conditioning_mask], dim=1)
        ...
        # 6. Denoising loop
        # ✅ DEBUG: Print blending status before loop starts
        if hasattr(self, '_using_heatmap_blending') and self._using_heatmap_blending:
            logger.info("🔀 " + "="*78)
            logger.info("🔀 USING HEATMAP BLENDING")
            logger.info("🔀 " + "="*78)
            logger.info(f"🔀 Inference steps: {num_inference_steps}")
            logger.info(f"🔀 Blend weights per step:")
            # Check if using 3-tuple (with latent B) or 2-tuple (without latent B)
            has_latent_b = self._auxiliary_latents_b_packed is not None
            for step_idx, weights in enumerate(self._blend_weights):
                if has_latent_b:
                    aux_a_w, aux_b_w, noise_w = weights
                    logger.info(f"🔀   Step {step_idx}: auxA={aux_a_w:.2f}, auxB={aux_b_w:.2f}, noisemap={noise_w:.2f}")
                else:
                    aux_w, noise_w = weights
                    logger.info(f"🔀   Step {step_idx}: auxiliary={aux_w:.2f}, noisemap={noise_w:.2f}")
            logger.info("🔀 " + "="*78)
        else:
            logger.info("ℹ️  Standard inference without heatmap blending")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ✅ NEW: Blend noisemap latents per step if auxiliary latents and blend weights are provided
                if (
                    generate_noisemap
                    and reference_video is not None
                    and self._auxiliary_latents_packed is not None
                    and noisemap_blend_weights is not None
                ):
                    # Get weights for current step
                    # Check if using 3-tuple (with latent B) or 2-tuple (without latent B)
                    if self._auxiliary_latents_b_packed is not None:
                        # Three-way blending: auxA + auxB + noisemap
                        aux_a_weight, aux_b_weight, noise_weight = noisemap_blend_weights[i]

                        # ✅ DEBUG: Log blending at first and last step
                        if i == 0:
                            logger.info(f"🔀 Step {i}/{num_inference_steps-1}: Blending with auxA={aux_a_weight:.2f}, auxB={aux_b_weight:.2f}, noise={noise_weight:.2f}")
                        elif i == num_inference_steps - 1:
                            logger.info(f"🔀 Step {i}/{num_inference_steps-1}: Blending with auxA={aux_a_weight:.2f}, auxB={aux_b_weight:.2f}, noise={noise_weight:.2f}")

                        # Blend all three latents
                        blended_noisemap = (
                            aux_a_weight * self._auxiliary_latents_packed +
                            aux_b_weight * self._auxiliary_latents_b_packed +
                            noise_weight * self._noisemap_latents_base
                        )
                    else:
                        # Two-way blending: aux + noisemap (backward compatibility)
                        aux_weight, noise_weight = noisemap_blend_weights[i]

                        # ✅ DEBUG: Log blending at first and last step
                        if i == 0:
                            logger.info(f"🔀 Step {i}/{num_inference_steps-1}: Blending with aux={aux_weight:.2f}, noise={noise_weight:.2f}")
                        elif i == num_inference_steps - 1:
                            logger.info(f"🔀 Step {i}/{num_inference_steps-1}: Blending with aux={aux_weight:.2f}, noise={noise_weight:.2f}")

                        # Blend auxiliary and noisemap latents
                        blended_noisemap = (
                            aux_weight * self._auxiliary_latents_packed + noise_weight * self._noisemap_latents_base
                        )

                    # Update the noisemap portion in latents
                    # latents structure: [reference_latents, frame_conditions, target_latents, noisemap_latents]
                    # We need to replace the noisemap portion
                    latents_without_noisemap = latents[:, :-noisemap_num_latents]
                    latents = torch.cat([latents_without_noisemap, blended_noisemap], dim=1)

                    # ✅ CRITICAL FIX: Also update init_latents with the blended noisemap
                    # Otherwise add_noise_to_image_conditioning_latents will overwrite with original noisemap
                    if init_latents is not None:
                        init_latents_without_noisemap = init_latents[:, :-noisemap_num_latents]
                        init_latents = torch.cat([init_latents_without_noisemap, blended_noisemap], dim=1)
        ...
        if reference_video is not None and output_reference_comparison:
            ...
            # ✅ MODIFIED: Regular processing - remove conditioning parts and output generated video
            if reference_video is not None:
            ...
            # ✅ MODIFIED: Remove noisemap latents (they are conditioning, not output)
            if generate_noisemap and reference_video is not None:
                # Remove noisemap latents from the end
                latents = latents[:, :-noisemap_num_latents]

            # ✅ Unpack latents (only GT, no noisemap)
        ...
        # ✅ MODIFIED: Return only generated video (noisemap is not generated, it's conditioning input)
        if not return_dict:
            return (video,)
