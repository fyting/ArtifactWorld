...
def read_video_for_validation(video_path: str | Path, num_output_frames: int) -> tuple[Tensor, float]:
    """Load reference or noisemap video the same way as ``validate`` / ``_sample_videos``.

    Uses ``read_video(..., target_frames=None)`` to decode all frames with decord; **only if** the
    length exceeds ``num_output_frames`` we keep the first ``num_output_frames`` frames. No uniform
    temporal resampling; shorter clips keep their original length.

    Note: an older bug used ``read_video(path)[:num_output_frames]``, which slices the
    ``(tensor, fps)`` tuple instead of the tensor, so the full-length tensor was always passed and
    ``trim_conditioning_sequence`` in ``ltxv_pipeline`` capped to ``num_frames``. Short clips
    (e.g. 89-frame heatmap vs 97 ``video_dims``) therefore stay at 89 frames, matching auxiliary
    latents exported at 89 frames.
    """
    frames, fps = read_video(video_path, target_frames=None)
    if frames.shape[0] > num_output_frames:
        frames = frames[:num_output_frames]
    return frames, fps
