...
from ltxv_trainer.video_utils import read_video_for_validation
...
class LtxvTrainer:
        ...
        with live:
            ...
            for step in range(cfg.optimization.steps * cfg.optimization.gradient_accumulation_steps):
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    # Handle dict loss (for noisemap training) or scalar loss
                    if isinstance(loss, dict):
                        loss_dict = loss
                        loss_scalar = loss_dict['total']
                    else:
                        loss_dict = None
                        loss_scalar = loss

                    self._accelerator.backward(loss_scalar)
                    ...
                    if IS_MAIN_PROCESS:
                        ...
                        # Add loss metrics (detailed if noisemap, otherwise single loss)
                        if loss_dict is not None:
                            metrics["train/loss"] = loss_dict['gt'].item()  # GT loss keeps original name for comparison
                            metrics["train/loss_total"] = loss_dict['total'].item()
                            # MODIFIED: No longer logging noisemap loss (noisemap is conditioning now)
                            # metrics["train/loss_noisemap"] = loss_dict['noisemap'].item()
                        else:
                            metrics["train/loss"] = display_loss

                        self._log_metrics(metrics)

                        if disable_progress_bars and self._global_step % 20 == 0:
                            if loss_dict is not None:
                                # MODIFIED: Simplified logging (no noisemap loss)
                                logger.info(
                                    f"Step {self._global_step}/{cfg.optimization.steps} - "
                                    f"Loss: {display_loss:.4f} (GT: {loss_dict['gt'].item():.4f}), "
                                    f"LR: {current_lr:.2e}, "
    ...
    def _load_models(self) -> None:
        ...
        components = load_ltxv_components(
            ...
            scheduler_source=self._config.model.scheduler_source,
            tokenizer_source=self._config.model.tokenizer_source,
            text_encoder_source=self._config.model.text_encoder_source,
            vae_source=self._config.model.vae_source,
            transformer_source=self._config.model.transformer_source,
    ...
    def _sample_videos(self, progress: Progress) -> list[Path] | None:
        ...
        use_images_end = self._config.validation.images_end is not None

        # print('[info], ', use_images)
        ...
        # Transfer noisemap_feature_scale to pipeline for train-inference consistency
        if self._config.conditioning.generate_noisemap:
            pipeline.noisemap_feature_scale = self._config.conditioning.noisemap_feature_scale
            # logger.info(f"Set pipeline noisemap_feature_scale to {pipeline.noisemap_feature_scale}")
        ...
        for j, prompt in enumerate(self._config.validation.prompts):
            ...
            # Load and add images (first frame and/or last frame), if provided
            cond_images = []
            cond_frame_indices = []
            ...
            if use_images:
                ...
                cond_frame_indices.append(0)

            if use_images_end:
                # import pdb; pdb.set_trace()
                image_path = self._config.validation.images_end[j]
                image = open_image_as_srgb(image_path)
                if image.size != (width, height):
                    # Resize and center crop the image to match the validation video dimensions
                    image = F.resize(image, size=min(width, height))
                    image = F.center_crop(image, output_size=(width, height))
                cond_images.append(image)
                cond_frame_indices.append(frames - 1)

            if cond_images:
                pipeline_inputs["image"] = cond_images
                pipeline_inputs["frame_index"] = cond_frame_indices

            # Load and add reference video, if provided
            reference_video_name = None  # Track reference video name for output naming
            if self._config.validation.reference_videos is not None:
                video_path = self._config.validation.reference_videos[j]
                # Extract the original video filename (without path)
                from pathlib import Path as PathLib
                reference_video_name = PathLib(video_path).name

                ref_video, _ = read_video_for_validation(video_path, frames)
                pipeline_inputs["reference_video"] = ref_video

                # MODIFIED: Load noisemap video if configured (instead of generating random noise)
                if self._config.conditioning.generate_noisemap:
                    if self._config.validation.noisemap_videos is None:
                        raise ValueError("generate_noisemap=True requires noisemap_videos to be specified in validation config")

                    noisemap_path = self._config.validation.noisemap_videos[j]
                    noisemap_video, _ = read_video_for_validation(noisemap_path, frames)
                    pipeline_inputs["noisemap_video"] = noisemap_video
                    pipeline_inputs["generate_noisemap"] = True

                    # NEW: Load auxiliary latent A for blending if configured
                    if self._config.validation.auxiliary_latent_path is not None:
                        pipeline_inputs["auxiliary_latent_path"] = self._config.validation.auxiliary_latent_path

                    # NEW: Load auxiliary latent B for blending if configured
                    if self._config.validation.auxiliary_latent_b_path is not None:
                        pipeline_inputs["auxiliary_latent_b_path"] = self._config.validation.auxiliary_latent_b_path

                    # NEW: Pass blend weights if configured
                    if self._config.validation.noisemap_blend_weights is not None:
                        pipeline_inputs["noisemap_blend_weights"] = self._config.validation.noisemap_blend_weights

                    # NEW: Pass save noisemap latents config
                    if self._config.validation.save_noisemap_latents:
                        pipeline_inputs["save_noisemap_latents"] = True
                        # Generate save path based on noisemap video name
                        from pathlib import Path as PathLib
                        noisemap_video_name = PathLib(noisemap_path).stem
                        latents_save_path = output_dir / f"noisemap_latents_{noisemap_video_name}.pt"
                        pipeline_inputs["noisemap_latents_save_path"] = str(latents_save_path)

                # Save reference video separately (DISABLED - not needed)
                # ref_video_path = output_dir / f"ref_step_{self._global_step:06d}_{i}.mp4"
                # from ltxv_trainer.video_utils import save_video
                # save_video(ref_video, ref_video_path, fps=24)

                # Save noisemap video if provided (DISABLED - not needed)
                # if self._config.conditioning.generate_noisemap:
                #     noisemap_video_path = output_dir / f"noisemap_step_{self._global_step:06d}_{i}.mp4"
                #     save_video(noisemap_video, noisemap_video_path, fps=24)

            with autocast(self._accelerator.device.type, dtype=torch.bfloat16):
                result = pipeline(**pipeline_inputs)

            # MODIFIED: Only handle generated video output (noisemap is input, not output)
            videos = result.frames

            for video_idx, video in enumerate(videos):
                if self._config.validation.remove_last_n_frames is not None and self._config.validation.remove_last_n_frames > 0:
                    frames_to_remove = self._config.validation.remove_last_n_frames
                    if len(video) > frames_to_remove:
                        video = video[:-frames_to_remove]
                        logger.debug(f"Removed last {frames_to_remove} frames from generated video.")
                    else:
                         logger.warning(f"Video has {len(video)} frames, cannot remove {frames_to_remove} frames. Skipping removal.")

                # Save generated video with reference video name if available
                if reference_video_name:
                    video_path = output_dir / reference_video_name
                else:
