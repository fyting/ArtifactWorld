    ...
    def _sample_videos(self, progress: Progress) -> list[Path] | None:
        ...
        for j, prompt in enumerate(self._config.validation.prompts):
            ...
            if self._config.validation.reference_videos is not None:
                ...
                # Extract filename from reference video path
                ref_video_name = Path(video_path).name

                # Removed: Save reference video separately (not needed)
                # ref_video_path = output_dir / f"ref_{ref_video_name}"
            ...
            for video in videos:
                ...
                # Replace last 9 frames with black frames
                if len(video) > 9:
                    # video is a list of PIL Images or numpy arrays
                    # Get the shape of a frame to create black frames
                    if hasattr(video[0], 'size'):  # PIL Image
                        width, height = video[0].size
                        black_frame = Image.new('RGB', (width, height), (0, 0, 0))
                        for frame_idx in range(len(video) - 9, len(video)):
                            video[frame_idx] = black_frame.copy()
                    else:  # numpy array
                        frame_shape = video[0].shape
                        black_frame = np.zeros(frame_shape, dtype=video[0].dtype)
                        for frame_idx in range(len(video) - 9, len(video)):
                            video[frame_idx] = black_frame.copy()
                    logger.debug("Replaced last 9 frames with black frames.")

                # Use reference video name if available, otherwise use default naming
                if ref_video_name is not None:
                    video_path = output_dir / ref_video_name
                else:
            ...
            if first_video_inference:
                logger.info(f"⏱️  First video inference completed in: {video_time:.2f}s")
                first_video_inference = False
            elif j % 10 == 9:  # Log every 10 videos
                logger.info(f"⏱️  Video {j+1} inference time: {video_time:.2f}s")
